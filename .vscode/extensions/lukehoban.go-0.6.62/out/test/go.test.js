...
`;
    ...
    test('Error checking', (done) => {
        ...
        util_1.getGoVersion().then(version => {
            ...
            return goCheck_1.check(path.join(fixturePath, 'errorsTest', 'errors.go'), config).then(diagnostics => {
    ...
    test('Test Outline', (done) => {
        ...
        goOutline_1.documentSymbols(options).then(outlines => {
    ...
    test('Workspace Symbols', (done) => {
        ...
        let withoutIgnoringFolders = goSymbol_1.getWorkspaceSymbols(workspacePath, 'WinInfo', configWithoutIgnoringFolders).then(results => {
            assert.equal(results[0].name, 'WinInfo');
            assert.equal(results[0].path, path.join(workspacePath, 'vendor/9fans.net/go/acme/acme.go'));
        });
        let withIgnoringFolders = goSymbol_1.getWorkspaceSymbols(workspacePath, 'WinInfo', configWithIgnoringFolders).then(results => {
